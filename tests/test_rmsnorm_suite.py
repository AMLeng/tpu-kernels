...
def test_block_default_matches_kernel_default_block() -> None:
    """`uv run python -m benchmarks.suites.rmsnorm` (no flags) must use the
...
def test_block_rejects_wrong_axis_count() -> None:
    """rmsnorm tiles rows in 1D; a 2-axis ``--block`` is a typo against
...
def test_sweep_block_rejects_wrong_axis_count() -> None:
    ...
    crash deep inside ``rmsnorm_pallas``."""
    parser = _make_parser()
